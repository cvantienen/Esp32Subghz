...
def scan_flipper_directory(input_dir: Path) -> list[SignalCategory]:
    ...
    if not input_dir.exists():
        logger.error("[ERROR] Input directory does not exist: %s", input_dir)
        return []
...
def generate_header(categories: list[SignalCategory]) -> str:
    ...
    for category in categories:
        array_name = (
            f"GEN_{sanitize_identifier(category.name).upper()}_SIGNALS"
        )
...
def generate_source(categories: list[SignalCategory]) -> str:
    ...
    for category in categories:
        ...
        count_name = f"NUM_GEN_{sanitize_identifier(category.name).upper()}"
        SOURCE_CODE.append(
            f"const uint8_t {count_name} = sizeof({array_name}) / sizeof(SubGHzSignal);"
        )
        SOURCE_CODE.append("")
