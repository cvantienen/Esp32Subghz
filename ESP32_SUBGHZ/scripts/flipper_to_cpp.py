...
def scan_flipper_directory(input_dir: Path) -> list[SignalCategory]:
    ...
    if not input_dir.exists():
        print(f"[INFO] Input directory '{input_dir}' does not exist. Creating it...")
        input_dir.mkdir(parents=True, exist_ok=True)
        # Create example structure
        (input_dir / "Example").mkdir(exist_ok=True)
        print(f"[INFO] Created '{input_dir}' with 'Example' subdirectory.")
        print("[INFO] Add your .sub files and re-run the script.")
        return []
...
def generate_header(categories: list[SignalCategory]) -> str:
    ...
    for category in categories:
        for signal in category.signals:
            ...
            lines.append(f"constexpr uint16_t {const_name} = {len(signal.raw_data)};")
...
def generate_source(categories: list[SignalCategory]) -> str:
    ...
    lines.extend(["// ==================== SIGNAL MENU STRUCTURES ====================", ""])
    ...
    for category in categories:
        ...
        count_name = f"NUM_GEN_{sanitize_identifier(category.name).upper()}"
        lines.append(f"const uint8_t {count_name} = sizeof({array_name}) / sizeof(SubGHzSignal);")
        lines.append("")
    ...
    for i, category in enumerate(categories):
        ...
        lines.append(f'    {{"{safe_name}", {array_name}, {count_name}}}{comma}')
...
def main():
    ...
    print("=" * 60)
    print("Flipper Zero SubGHz to C++ Converter")
    print("=" * 60)
    print(f"Project root:  {project_root}")
    print(f"Input dir:     {input_dir}")
    print(f"Output header: {project_root / OUTPUT_HEADER}")
    print(f"Output source: {project_root / OUTPUT_SOURCE}")
    print("=" * 60)
    print()
    ...
    if not categories:
        print()
        print("[INFO] No signals found. Add .sub files to the input directory.")
        print("[INFO] Expected structure:")
        print(f"       {input_dir}/")
        print("       ├── CategoryName/")
        print("       │   ├── signal1.sub")
        print("       │   └── signal2.sub")
        print(f"       └── uncategorized.sub  (goes to '{MISC_CATEGORY}')")
        return 0
    ...
    total_signals = 0
    for category in categories:
        print(f"  {category.name}: {len(category.signals)} signal(s)")
        total_signals += len(category.signals)
    print(f"  Total: {total_signals} signal(s) in {len(categories)} category(ies)")
    print()
    ...
    print()
    print("To use the generated signals, include the header in your code:")
    print('    #include "generated_signals.h"')
    print()
    print("Access signals via:")
    print("    GEN_SIGNAL_CATEGORIES[i].signals[j]")
    print("    NUM_GEN_CATEGORIES")
